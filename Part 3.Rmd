...
arg <- st_write(argentina, "./Produtos/argentina.shp", delete_dsn = TRUE)
chl <- st_write(chile, "./Produtos/chile.shp", delete_dsn = TRUE)
bol <- st_write(bolivia, "./Produtos/bolivia.shp", delete_dsn = TRUE)
...
poa_sf <- st_as_sf(poa_df, coords = c("x", "y"))
st_write(poa_sf,"./Produtos/mdt.shp", delete_dsn = TRUE)
elmat <- raster_to_matrix(elev_poa)
elmat[is.na(elmat)] <- 1

water_layer <- elmat < 5

water_layer <- water_layer[nrow(water_layer):1, ]

elmat %>%
  sphere_shade(texture = "desert") %>%
  add_water(water_layer, color = "lightblue") %>%
